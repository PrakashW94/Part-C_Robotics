...
void avoidBox()
{
	...
	while (1)
	{	
		fp = (int)((e_get_prox(7) + e_get_prox(0))/2);
		// We are close to object
		reportValue("fp", fp);
		while (fp > 300)
		{
			// Rotate Right until front sensors no longer detect box
			setSpeed(400, -400);
			fp = (int)((e_get_prox(7) + e_get_prox(0))/2);
			wait(delayTimer);
			reportValue("fp", fp);
		...
		// Move forwards until side is no longer detected
		lp = getLeftProx();
		while (lp > 300)
		{
			setSpeed(500, 500);
